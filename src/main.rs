...
use keiros::feature_loader::generate_loader;
...
fn main() -> Result<()> {
    let cli = Cli::parse();
    match cli.command {
        Commands::Init {agent_name, no_cargo}=> init_agent_structure(agent_name.as_deref(), no_cargo)?,
